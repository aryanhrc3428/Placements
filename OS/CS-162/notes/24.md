...
* **The Core Constraint:** Two military generals must coordinate a joint attack. If they attack at different times, they will be isolated and defeated; if they attack together, they win. They can only communicate by sending messengers through a valley where they risk capture. (Note: The paradox is named after Custer, who died at Little Big Horn because he arrived a couple of days too early).
...
### 7.1 Simple Data Types and Machine Representation
Consider a simple data type `uint32_t x;` that you want to write to a file (`FILE* f = fopen("foo.txt", "w");`). You have two choices:
1. **Text format:** `fprintf(f, "%lu", x);`
2. **Machine representation (Binary):** `fwrite(&x, sizeof(uint32_t), 1, f);` or `write(fd, &x, sizeof(uint32_t));`

Neither is inherently "wrong", but the sender and receiver must be consistent. Using the machine representation is faster and more compact, but introduces complications when reading it back on a different machine, specifically regarding endianness.

### 7.2 The Endianness Bottleneck
...
* **Dynamic Binding via Name Services:** The system resolves endpoints at runtime using a centralized registration directory called a **Name Service**. When a server starts up, it registers its active mailbox endpoint with the name service. When a client initializes, it queries the name service to fetch a healthy server's current address. 
  ...
  This dynamic lookup provides major operational benefits, including **Access Control validation**  and automated **Fail-Over protection** (directing traffic away from crashed servers). It also simplifies **Load Balancing**, allowing the name service to distribute new client sessions across a pool of unloaded servers (e.g., passing a pointer to a client-specific return mbox in the request).

### 8.5 Cross-Platform Issues and RPC Details
When client and server machines have different architectures or languages:
* Everything must be converted to/from some canonical form.
* A common optimization is to **tag every item with an indication of how it is encoded**. This allows the system to avoid unnecessary conversions if the architectures happen to match.

### 8.6 Distributed Failure Modes and Performance Realities
...
### 8.7 Cross-Domain Communication and Location Transparency
...
RPCs can be used to communicate between address spaces on *different machines* or the *same machine*. Services can be run wherever it is most appropriate, and access to local and remote services looks identically transparent.
Examples of RPC systems include:
* **CORBA** (Common Object Request Broker Architecture)
* **DCOM** (Distributed COM)
* **RMI** (Java Remote Method Invocation)

### 8.8 Cross-Domain Microkernel Implementations
