...
> **Key Assumptions:** The system is in equilibrium, and there is no limit to the queue length.
...
* **$L_q$ (Length of Queue):** The average number of requests waiting in the queue. By Little's Law, $L_q = \lambda \times T_Q$.
...
### 2.0 Characteristics of Files
Based on a study published in FAST 2007, real-world file system workloads exhibit two key observations that drive filesystem design:
1. **Observation #1: Most Files Are Small.** This means the filesystem must efficiently store many small files without wasting significant metadata space.
2. **Observation #2: Most Bytes are in Large Files.** This means the filesystem must also be capable of providing high-throughput access to massive files.

### 2.0.1 Critical Factors in File System Design
* **Disk Performance (!!!):** The core goal is to maximize sequential access and minimize mechanical seeks.
* **Open before Read/Write:** Opening a file in advance allows the OS to perform protection checks and look up the file's resource locations before any data transfer begins.
* **Size is Determined on the Fly (!!!):** File size is determined as it is used. The filesystem must allow files to start small and grow organically by writing (or reading zeros) to expand.
* **Organized into Directories:** Requires robust on-disk data structures.
* **Careful Block Allocation / Freeing:** Blocks must be allocated and freed in a way that keeps subsequent access efficient.
...
#### UNIX 4.1 BSD File System Pros and Cons
* **Pros:** Simple (more or less). Files can easily expand (up to a point). Small files are particularly cheap and easy to access, while large files can still be handled!
* **Cons:** Lots of seeks are required (metadata vs data). Very large files must read many indirect blocks (e.g., four I/Os per block for triple indirect!).
...
>
> **Cons:**
> * Inefficient for tiny files (e.g., a 1-byte file requires both a full inode and a full data block).
