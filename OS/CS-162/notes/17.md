...
> **Key Insight: MIPS R3000 Pipeline & Segments**
> To make physically-indexed caches fast, the TLB lookup is integrated into the processor pipeline:
> - **Instruction Fetch:** TLB $\rightarrow$ I-Cache
> - **Decode / Register Fetch:** RF
> - **ALU / Effective Address (E.A.):** Operation & E.A. Calculation
> - **Memory Access:** TLB $\rightarrow$ D-Cache
> - **Write Back (WB):** Write Reg
> 
> The MIPS R3000 Virtual Address Space divides segments as follows:
...
> 
> A 6-bit ASID allows context switching among 64 user processes without requiring a TLB flush.
...
### 5.1 The Illusion of Infinite Memory
Because disk storage is vastly larger than physical memory, the combined virtual memory footprint of all running processes can easily exceed available RAM. This allows more programs to fit into memory simultaneously, maximizing concurrency. 

> **Key Insight:** Virtual memory relies on a **Transparent Level of Indirection** (the page table). This supports the flexible placement of physical data—whether in local RAM, on disk, or across a network—completely transparently to the user program. The location of the data is a performance issue, not a correctness issue.

### 5.2 Review: What is in a Page Table Entry (PTE)?
A PTE typically contains a pointer to the next-level page table or the actual physical page frame, alongside permission bits (valid, read-only, read-write, write-only). 

For example, the **Intel x86 architecture PTE** (using a 2-level 10-10-12 bit offset structure with intermediate "Directories") defines the following bits:
* **P (Present):** Same as the "valid" bit in other architectures.
* **W (Writeable):** Indicates if the page can be written to.
* **U (User accessible):** Controls user-mode access.
* **PWT (Page Write Transparent):** Enforces external cache write-through.
* **PCD (Page Cache Disabled):** Page cannot be cached.
* **A (Accessed):** Indicates the page has been accessed recently.
* **D (Dirty):** Indicates the page has been modified recently (PTE only).
* **PS (Page Size):** If `PS=1`, it denotes a 4MB huge page (directory only, where the bottom 22 bits serve as the offset).
...
> **[Diagram]: Cache Behavior under WS Model**
> When evaluating hit rate against cache size, the curve shows that once the new working set fits into the cache, the hit rate approaches 1. The miss rate is composed of capacity, conflict, and compulsory misses. This cost is amortized by the fraction of time the Working Set is active before the program transitions from one working set to the next. This model applies equivalently to hardware memory caches and OS pages.
...
### 6.7 Origins and Modern Evolution of Paging
* **Origins of Paging:** Historically, disks provided the vast majority of storage, while memory was small. Systems hosted many clients on dumb terminals running different programs, requiring the OS to actively swap pages to and from disk and keep memory full of only the most frequently accessed pages.
* **The Modern Situation:** Today's systems feature powerful processors, huge memory, huge disks, and typically single-user environments. Memory typically stays about 75% used, with 25% reserved for dynamic allocations. A large portion of memory (e.g., 1.9 GB in example traces) is often shared.

### 6.8 Many Uses of Virtual Memory and Demand Paging
Beyond simple disk swapping, demand paging enables several core OS mechanisms:
* **Extend the Stack:** Automatically allocate a new page and zero it when the stack grows.
* **Extend the Heap:** Historically via `sbrk`, today primarily through `mmap`.
* **Process Fork:** Create a copy of the page table where entries refer to the parent's pages as NO-WRITE. Shared read-only pages remain shared, and modifications trigger a **Copy-on-Write** (CoW) page fault.
* **Exec:** Bring in parts of the binary executable only as they are actively used (on demand).
* **Memory-Mapped Files (MMAP):** Explicitly share memory regions or access a file transparently as if it were RAM.

### 6.9 Free Frame Management and Scheduling
When a page fault occurs, where does the OS get a free frame?
* The OS maintains a **free list** of available frames.
* In Unix, a background **"reaper"** process runs when memory gets too full. It schedules dirty pages to be written back to disk and zeroes out clean pages that haven't been accessed recently.
* As a last resort, the OS will directly evict a dirty page to fulfill an immediate allocation request.
* **Scheduling Memory:** Like thread scheduling, the OS must schedule memory resources by balancing utilization, fairness, priority, and the allocation of disk paging bandwidth among competing processes.
...
## 8. Lecture Administrivia & Extras
* **Upcoming Deadlines:** Midterm 2 is scheduled for 3/31 (the first Tuesday after Spring Break). Project 2 is in full swing—do not wait until the last moment. Homework 4 begins on Saturday.
* **Feedback:** Students are encouraged to fill out the course survey to provide feedback and suggest topics for special topics lectures.
* **Attendance Perk (3/19/2026):** Take a selfie with the screen and QR code in the background (showing both face and screen) before the end of class for extra credit points. Happy Spring Break!
