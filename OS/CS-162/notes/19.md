...
> **Note:** Allocation priorities in Linux dictate whether blocking is allowed during memory requests, among other constraints.
...
> **Key Insight:** Kernel memory is generally not visible to user space. One exception is the **VDSO (virtual dynamically linked shared objects)** facility, which maps specific kernel code directly into user space to aid in fast system calls (e.g., `gettimeofday()`).
...
* **Process Context Identifiers (PCID):** Constantly switching between dual page tables requires updating the `CR3` register, which historically forced a full flush of all non-global TLB entries. This introduced a severe **800% performance overhead penalty** during frequent system calls. Modern systems (requiring at least **Linux v4.14**) avoid this penalty by using **PCID tags**. The hardware appends a unique identifier tag to each TLB entry, allowing user and kernel translations to coexist safely without needing a full TLB flush during address space transitions.

> **Note:** While hardware fixes without timing side-channels largely resolved Meltdown, the related **Spectre** vulnerability relies on more fundamental speculative execution mechanisms and is significantly harder to fix.
...
1. **Interface Standardization:** Device drivers must provide a uniform API abstraction layered over thousands of unique, specialized hardware layouts. (e.g., How can we standardize interfaces?)
...
Processors communicate with internal device controllers (which contain readable/writable registers and memory for queues) using two primary architectural styles:
...
* **Frame Buffer:** Display memory (e.g., mapped from `0x8000F000` to `0x8000FFFF`). Writing to this physical memory changes the image on the screen.
* **Command Queue:** Graphics command queue (e.g., mapped from `0x80010000` to `0x8001FFFF`). The CPU can write a graphics description (like a set of triangles) to the command queue.
* **Command Register:** Writing to a specific command register (e.g., `0x0007F004`) tells the on-board graphics hardware to render the scene.
...
* **Programmed I/O (PIO):** The CPU performs every byte transfer manually using explicit loop constructs (via `in`/`out` or `load`/`store`). While simple to implement and requiring no specialized hardware support, PIO keeps the CPU pinned in a busy loop during transfers, **wasting processor cycles proportional to the total size of the data payload**.
* **Direct Memory Access (DMA):** The system delegates data transfers to a specialized hardware component called a **DMA Controller**, granting it access to the memory bus. The CPU initializes the transfer parameters and then immediately context switches to execute other application threads. The DMA controller transfers data blocks between the device and main memory directly over the system bus, completely independent of the CPU.
...
* **The Top Half:** Invoked directly when user applications execute system calls (e.g., `open()`, `read()`, `write()`, `ioctl()`, `strategy()`). The top half runs within the context of the calling process thread, validates arguments, updates request queues, initiates the hardware transfer, and **puts the thread to sleep** until the operation completes.
  * Note: The `ioctl()` system call allows for special device-specific configuration that doesn't fit standard interfaces.
* **The Bottom Half:** Triggered asynchronously by the hardware when a physical interrupt occurs. It runs within a generic interrupt context, retrieves data payloads from device registers or transfers the next block of output, clears status flags, and **wakes up the sleeping process thread** to complete the system call.

---

#### 10.2 Standard Interfaces to Devices
The kernel seeks to provide uniform interfaces despite a wide range of different devices.
* **Block Devices:** (e.g., disk drives, tape drives, DVD-ROM). Access data in blocks. Commands include `open()`, `read()`, `write()`, `seek()`. Support raw I/O or file-system access, as well as memory-mapped file access.
* **Character Devices:** (e.g., keyboards, mice, serial ports, some USB devices). Access single characters at a time. Commands include `get()`, `put()`. Libraries layered on top allow for line editing.
* **Network Devices:** (e.g., Ethernet, Wireless, Bluetooth). Different enough to require their own interface. Unix and Windows provide a **socket interface**, separating network protocol from network operation (includes `select()` functionality). Usage involves pipes, FIFOs, streams, queues, and mailboxes.

### 10.3 Structural I/O Timing Interfaces
...
* **Non-Blocking Interface ("Don't Wait"):** The system call returns immediately. If the device is busy or no data is available, it returns an error code (e.g., `EAGAIN`) alongside a count of successfully transferred bytes (which may be zero for read/write).
...
### 10.4 Operational Parameters for I/O
Devices are broadly categorized by how they handle operations:
* **Data Granularity:** Byte vs. Block (some devices provide single bytes like keyboards, others provide whole blocks like disks).
* **Access Pattern:** Sequential vs. Random (tape drives must be accessed sequentially, whereas disks/CDs can be accessed randomly). Random access typically incurs a fixed overhead to start transfers. Some devices require continual monitoring while others generate interrupts.

### 10.5 Mechanical Disk Performance Metrics
